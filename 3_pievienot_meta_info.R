...
# 1. Ielādē Meta failus ----
meta_sites <- read_csv("./rezultati/BirdnetR_meta_sites.csv", show_col_types = FALSE)
meta_sites_week <- read_csv("./rezultati/BirdnetR_meta_sites_week.csv", show_col_types = FALSE)
meta_latvia <- read_csv("./metadati/BirdnetR_meta_latvia.csv", show_col_types = FALSE)
...
# 2. Definē birdnet_week funkciju ----
...
# 4. Pievieno visas Meta kolonnas ----
...
all_results_list <- all_results_list %>%
  imap(~ {
    ...
    df <- .x %>%
      ...
      # 4. Pievieno site_week_confidence
      left_join(
