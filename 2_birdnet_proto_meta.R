...
analyze_folder <- function(audio_folder, 
  ...
  # Viens fails visiem overlap variantiem
  # Izveido mapi, ja tā neeksistē
  if (!dir.exists("proto")) {
    dir.create("proto")
  }
  
  # Noņem "ML_" prefiksu no folder_name
  clean_name <- str_replace(folder_name, "^ML_", "")
  csv_file <- paste0("./proto/all_results_", tolower(clean_name), ".csv")
  
  # Dzēš veco failu, ja eksistē (lai sāktu no jauna)
  ...
  for (overlap in overlap_values) {
    ...
    for (i in seq_along(batches)) {
      ...
      # Saglabā CSV (append mode, lai visi overlap būtu vienā failā)
      if (!file.exists(csv_file)) {
...
lookup_sites <- vietas %>%
  select(site_id, sitename) %>%
  mutate(
    name_clean = str_replace(sitename, "^ML_", "")
  ) %>%
  separate(name_clean, into = c("quadrant", "mic_type"), sep = "_", remove = TRUE) %>%
...
write_csv(results_meta_sites, "./rezultati/BirdnetR_meta_sites.csv")
...
write_csv(results_meta_sites_week, "./rezultati/BirdnetR_meta_sites_week.csv")